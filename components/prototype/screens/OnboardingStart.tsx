...
export function AuthScreen() {
  ...
  return (
    <section className="onboarding-screen auth-screen"><BrandLogo className="auth-logo" /><p className="step-label">필수 1 / 2</p><h1>계정으로<br />로그인해 주세요</h1><p className="muted">세 가지 방법 중 하나를 선택해야 계속할 수 있어요.</p><div className="provider-list"><button className="provider-button kakao" aria-label="카카오로 계속" onClick={() => login("kakao", "카카오")}><span aria-hidden="true">💬</span><b>카카오</b></button><button className="provider-button google" aria-label="Google로 계속" onClick={() => login("google", "Google")}><span aria-hidden="true">G</span><b>Google</b></button><button className="provider-button apple" aria-label="Apple로 계속" onClick={() => login("apple", "Apple")}><span aria-hidden="true">●</span><b>Apple</b></button></div><div className="auth-requirement"><b>다음 단계도 필수예요</b><p>로그인 후 학교 인증까지 완료해야 CampusDrop에 입장할 수 있어요.</p></div><p className="legal">계속하면 <button onClick={() => setTerms(true)}>이용약관 및 개인정보처리방침</button>에 동의하게 됩니다.</p>
